...
# ========== FastText Loading + Sentence Encoding ==========
...
def mean_pool_fasttext(sentence: str, ft: KeyedVectors, dim: int = 300):
...
def encode_sentences(sentences, ft, dim=300):
    X = []
    for s in tqdm(list(sentences), desc="Encoding (FastText mean pooling)"):
        X.append(mean_pool_fasttext(s, ft, dim=dim))
    return np.stack(X)

print("\n========== Encoding Sentences ==========")
X = encode_sentences(data['sentence'], ft, dim=EMB_DIM)
y = data['label'].values
...
class MLPClassifier(nn.Module):
    def __init__(self, input_dim, num_classes, hidden1=256, hidden2=64, dropout=0.5):
        super().__init__()
...
model = MLPClassifier(input_dim=EMB_DIM, num_classes=num_classes)
